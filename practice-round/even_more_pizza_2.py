...
with open(argv[1], 'r') as f_in:
  num_pizzas, num_teams_2, num_teams_3, num_teams_4 = [int(n) for n in f_in.readline().split()]
...
def pick_x_pizzas(x):
  ...
  for i in range(2, x+1):
    for ing in first_pizza.ings:
      filter_candidate_ids = [y for y in local_candidates_ids if y not in dict_ing[ing]]

...
with open(argv[1].split('.')[0]+".out", 'w+') as f_out:
    ...
    for intersec in list_intersec:
        ...
        for street in intersec.ins:
            if street_times[street] == 0 and intersec.num_ins > 1:
                intersec.ins.remove(street)
                intersec.num_ins -= 1
        ...
        # change!
            ...
            total = sum(street_times[street] for street in intersec.ins)
            if total == 0:
                for street in intersec.ins:
                ...
                for street in intersec.ins:
                    duration = min(
                        round((street_times[street] /
                               total * intersec.num_ins) + 1),
                        sim_dur
                    )
                    f_out.write(street + ' ' +
                                str(duration) + '\n')
